...
| `check-release.mjs`（09-18 新增，随「下载解压就能跑」一起立起来） | ✅ `--self-test`（**4 条合成样本**：令牌原样躺在 `.js` 里必须报、同一个令牌只出现在 `.png` 里不许报（二进制不在扫描范围）、干净文件不许报、样本里根本没有的值不许报（防乱报）。⚠️ 它的 A/B/C/E 四组是**环境断言** —— 文件在不在、`.gitignore` 有没有把它们挡住、服务起不起得来 —— 自证只覆盖 D 组那条纯函数判据。另做过一次**真实诱饵验证**：把 `.env` 里的真令牌写进 `dist/_leak-test.js`，D 组当场翻红，删掉后转绿。这条判据不是恒绿的壳） |
...
| `check-release.mjs`（09-18 新增） | **19 项，18 通过；`--self-test` 4 项全过**。唯一那条红是 C2「产物不比源码旧」，当轮状态是 `dist/` 还没提交（打印出来是「产物 从未提交」）—— 这正是它要抓的东西，提交后转绿。另有一次诱饵验证：真令牌写进 `dist/_leak-test.js` ⇒ D 组当场翻红，删除后转绿 |
