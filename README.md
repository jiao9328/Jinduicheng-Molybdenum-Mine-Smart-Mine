...
- **工程门禁** —— 20 个带退出码的巡检脚本 + 1 个发布前自检 + 7 个探针，把「面板被撑破」
  「图层没画出来」「坐标跑偏」「底图被悄悄删掉」「产物比源码旧」「凭据被内联进产物」
  这类**不报错的缺陷**变成可复现的红绿结论。
- **四张台账进了真库 + 登录分角色** —— 质检记录 / 值班与交接班 / 备件台账 / 隐患处置
  落在 SQLite 里，可在「数据管理」页增删改查；登录后分管理员与普通用户，
  **只有管理员能改**，且这个判定由后端逐个写接口强制（前端裁剪只是不让人白点）。
- **AI 助手「墩儿」（自然语言指挥层）** —— 右下角一句话驱动三维与业务系统：
  飞相机 / 切图层 / 查数据 / 建工单。**规则引擎优先、模型兜底**，核心指令离线 100% 可用；
  写操作走一次性确认令牌，**未经确认不写库**，每一次对话与每一次拒绝都记审计。
  它怎么搭起来的、为什么这么搭，见 [`docs/duner.md`](./docs/duner.md)。
...
| 🤖 墩儿（AI 助手） | 全站右下角 | **一句话驱动三维与业务**：`带我去选矿厂` 飞相机 / `只给我看边坡监测` 切图层 / `今天产量咋样` 调数据 / `生成处置工单…` 写操作**先弹确认卡再落库**。规则引擎优先、DeepSeek 可选兜底，核心指令**离线可用**；写操作走一次性令牌，**未经确认不写库**并记审计。实现说明见 [`docs/duner.md`](./docs/duner.md) |

## 📷 系统截图

> 以下截图由 `node scripts/make-screenshots.mjs` 生成，界面改动后重跑即可更新；
> 墩儿那两张由 `node scripts/make-duner-shots.mjs` 单独生成 —— 它要**驱动对话框**
>（点开按钮 → 发一句话 → 等回话 → 截），通用流程表达不了，理由写在脚本头。
...
**🤖 墩儿 · 一句话驱动三维** > 右下角悬浮按钮点开即对话框（按钮跟页面等比缩放，并**逐页量出落位**、让开右栏与页签）。
说「带我去选矿厂」→ 相机真的飞过去（**本图实测位移 3171 m**）、回话与画面能对上号；
`baseColor 0.0%` 也随图一起量过，不是空地球。
<img width="1347" alt="墩儿-对话框" src="./screenshots/duner-chat.png" />

**🤖 墩儿 · 写操作先弹确认卡** > 说「生成处置工单，对象尾矿库，事由…，责任人王建国」→ 回来的是**确认卡**
而不是直接建单：对象 / 原因 / 建议措施 / 责任人 / 紧急度 五行 + 倒计时。
**点确认之前一个字节都不写**（后端只发一枚一次性令牌），普通用户提出写意图**连卡都拿不到**；
这句话说完库里那条单子是 0 条 —— 截图脚本自己断言了这件事。
> 图里倒计时是 **172s** 而不是默认的 60s：软渲染下这一张全屏截图要 60~90 秒，
> 默认有效期的卡会在快门期间过期（第一版图里就掉到了「0s」），所以拍这张时
> 把 `DUNER_CONFIRM_TTL_MS` 临时配成 180s —— 顺带把「回话里写死 60 秒」这个缺陷配了出来（见第十三节第 36 条）。
<img width="1347" alt="墩儿-确认卡" src="./screenshots/duner-confirm.png" />
...
从 GitHub 下载 ZIP 解压后，**机器上有 Node ≥ 24 就行**，一条命令：

```bash
node server/index.mjs      # http://localhost:8787
```

**不用 `npm install`、不用联网、不用建库。** 仓库里带着三样东西让这条路成立：
构建好的 `dist/`（前端产物）、`public/cesium/`（Cesium 运行时资源）、
`server/data/mine.seed.db`（四张台账的种子基线）—— 首次启动会自动把种子库
复制成工作库 `mine.db`。打开 http://localhost:8787 ，用页面上写着的
`admin / admin123` 登录即可。

> ⚠️ **每个三维页面打开时会先白等约 15 秒**，然后一切正常。这是默认打开在线实景三维
> 的代价：它要连的 `tile.googleapis.com`（Google）在国内连不上，只能等超时再退回
> 程序化场景。**这条路子吃的是构建好的产物，改不了这个开关**（要改就走方式二并重新
> 构建）；它不影响任何功能，只是慢。来由与实测数字见[第十三节第 37 条](#十三已知限制)。

> 为什么能零依赖：后端是手写的 `node:http` + 内置 `node:sqlite`（见第十节），
> 一个第三方包都不引。代价是这条路子吃的是 `dist/` 这张**快照**，改不了源码。
>
> - 想回到干净基线：删掉 `server/data/mine.db` 再启动，会重新复制一份种子
> - 换端口 / 换库：`node server/index.mjs --port 9000 --db D:/tmp/x.db`
> - 改源码、重新构建、跑开发服务 → 走方式二
...
# ★ 一键启动（四张台账 + 登录 + 静态页）。要求 Node ≥ 24
npm run db:seed      # 可跳过：首次启动已自动从 mine.seed.db 复制出工作库。
                     # 需要「回到干净基线」时才手动跑（幂等：先清空再灌，会丢掉手工新增的记录）
npm run serve        # http://localhost:8787，同时托管 dist/ 与 /api
```

> **改完源码记得重新提交 `dist/`。** `dist/` 现在随仓库分发（方式一靠它），
> 而它**不会**自动跟着 `src/` 变 —— vite 的产物文件名带内容哈希，别人下到的
> 旧 `index.html` 指向的旧 chunk 与源码对不上，页面会以各种莫名其妙的方式坏掉。
> 提交前跑 `node scripts/check-release.mjs`，它会把「dist/ 过期」与
> 「凭据混进产物」拦下来。⚠️ 注意口径：`VITE_CESIUM_ION_TOKEN` 是**刻意公开**的
> （见 `.env.example` 里的白名单说明），D 组管的是**白名单之外**的保密值
> ——`DUNER_LLM_KEY` 那类——**一个都不许进产物**；同时它也**正向**要求公开令牌
> 必须在产物里，因为缺了它不会报错，只会静默退回程序化场景。
...
> **要完整跑起来（含登录）就 `npm run serve`** —— 库由首次启动自动复制种子、
> 构建由 `serve` 按需代劳，都不必手动做。
...
```
├── dist/                        前端构建产物（**随仓库分发** —— 方式一直接吃这张快照）
│                                改了 src/ 必须重新 `npm run build` 并提交，见第二节末的提醒
├── public/
│   ├── cesium/                  Cesium 运行时资源（postinstall 生成，**随仓库分发**）
...
│   ├── duner/                   墩儿（AI 助手）后端 —— 规则引擎 + 模型兜底，见第十三节
│   │   ├── config.mjs           读 .env + 开关判定（模型是否可用）
│   │   ├── dict.mjs             术语词典（种子 + 库里可热更的词条）
│   │   ├── normalize.mjs        术语与编号归一化（"边皮廊"→皮带廊、"三号"→3号）
│   │   ├── intents.mjs          意图体系（指导书 §6 的 19 条意图）
│   │   ├── rule.mjs             规则引擎：有序规则表 + 实体抽取 + 置信度
│   │   ├── llm.mjs              DeepSeek function calling 适配（工具名点号↔下划线）
│   │   ├── policy.mjs           角色校验（写操作强制管理员）
│   │   ├── confirm.mjs          写操作确认令牌（一次性 + TTL）
│   │   ├── audit.mjs            全链路审计（原话 → 意图 → 工具 → 结果）
│   │   ├── service.mjs          编排：NLU → 决策 → 权限 → 确认/执行 → 反馈
│   │   ├── facts.mjs            业务事实（打包前端 mock，与页面同源）
│   │   └── tools/               工具注册表（视图类只产出指令，不碰 Cesium）
│   └── data/
│       ├── mine.seed.db         种子基线（**入库**，只读不写；首次启动复制成工作库）
│       └── mine.db              工作库（**不入库**，运行时一直在写，见 .gitignore）
...
│   ├── check-duner.mjs          墩儿端到端巡检（7 条必过用例 + ⑧ 点名/方位/纠错 8 条
│   │                            + 295 条测试集算准确率/F1 + 真浏览器驱动相机/图层/确认卡
│   │                            + 右下角按钮十页逐页让位 + 17 个工具冒烟，`--self-test` 自证）
│   ├── fixtures/duner-cases.json 测试集（机械生成 244 条 + 手写 51 条，含口语/错别字/注入）
│   ├── check-release.mjs        发布前自检（下载即跑 / dist 是否过期 / 凭据有没有被内联进产物）
│   │                            —— **不开浏览器、不依赖 node_modules 与网络**，它查的正是这两样
│   │
...
│   ├── duner/                   墩儿（AI 助手）前端
│   │   ├── bus.ts               场景总线（viewer / flyTo / 图层的模块级注册表）
│   │   ├── bridge.ts            执行后端下发的指令 → 调 Cesium / 视图状态 → 回执
│   │   ├── places.ts            地名 → 机位（复用 sceneTargets 与 mock，不另建坐标表）
│   │   └── scene.ts             视图对总线的登记接口
│   ├── scene/                   Cesium 场景层
...
└── 参考/                        需求方提供的原始资料
    ...
    └── AI助手指导书_墩儿_自然语言驱动智慧矿山平台_Java后端版.md  AI 助手的能力清单与验收指标
                                                （**不在 docx 条目内**，见第十四节；技术选型按本仓库改）
```
...
| `MapScene` | Cesium 三维容器，可传入自定义场景构建器与初始机位（挂载时把自己的 `viewer`/`flyTo` 登记到墩儿的场景总线） |
| `PlanProgress` | 生产计划进度卡组 |
| `DunerDock` | 墩儿的**右下角悬浮按钮**（点开才是对话框）+ 对话框 + 确认卡片 + 澄清选项。挂在 `ScaleScreen` **内部**，所以它跟着 1920×1080 的缩放走（66px 的按钮在 1440×900 下量到 49.5px，见 `check-duner.mjs`）。按钮落位**当场量**、不写死：九页里有五页的右下角压着右栏或页签，它会向左让过挡路的面板（`src/duner/corner.ts`），十页的实测落位与三处已知代价见第十三节第 35 条 |
...
| `check-all-pages.mjs` | ✅ `--self-test`（**16 项** = 分类 6 + 结论行 10。分类段验「404 → 接口未就绪」以及 `外网不可达` 豁免**收得够窄**（没有失败记录不许豁免、豁免不吞同页其它错误、文本对不上不许豁免）；结论行段验全页健康、**99 条「接口未就绪」**（后端没起的预期降级）都不许报；真错误 / 渲染失败 / 截图失败 / 三维报错面板各必报；**一行结论都没有、少跑一页、结论不是数组**必报——巡检空转时 `[]` 让原判据恒绿） |
...
| `check-duner.mjs`（09-16~17 新增，AI 助手端到端；09-17 加右下角按钮与十页让位后重跑） | ✅ `--self-test`（**在真实链路上注入**：T1 把 `camera.flyTo` 换成"不飞但照常回调"的空函数 ⇒ 同一句话相机必须原地不动、还原后必须又能飞（3559 m）；T2 把目的地钉死成同一个点 ⇒ 两个不同地名的落点必须重合（这条判据本身若恒真，说明"落点由地名决定"是假的）；T4 从**前端源码里**扣掉一个地名 ⇒ 漂移比对必须当场红；T5 现注册一个**必崩**的工具 ⇒ 七段那句「工具都不许崩」必须发现它（另配一条反例：参数不齐时那句「要找谁？」是正常的 error，不许算成崩）；T6 注册一个**只有给了参数才崩**的工具 ⇒ 「空参数不许崩」那条确实抓不到它（这正是七段补"合法参数真跑一遍"的理由），而新的「最小合法参数」判据必须当场红；T7 手动把 `--duner-right` 扳回 16px ⇒ 十页扫描同一把量法必须当场看见按钮压住页签（否则那段扫描是恒绿的壳），一次点击还原后又必须什么都不压；T3 是反向期望：把「删除所有数据」期望成"产生了指令"、把「带我去尾矿库」期望成 `LAYER.SHOW`、把「导出工单」期望成 `ORDER.CREATE`，三条都必须站不住。⚠️ T1 第一版踩了两个坑，**两个都是"自证"抓出来的**：① 空函数没回调 `complete` ⇒ 前端 `busy` 一直挂着，后面每次输入都被挡掉（判红的看起来是"注入没还原"）；② 注入用的那句话（「带我去尾矿库」）与上一段结束时相机所在处**正好是同一个地方**，于是那条断言当时恒真） |

| `check-release.mjs`（09-18 新增，随「下载解压就能跑」一起立起来） | ✅ `--self-test`（**10 条合成样本**，分两段。**分类段 5 条**：没见过的变量默认归保密（默认保密、不是默认公开）、`DUNER_LLM_KEY` 归保密、`VITE_CESIUM_ION_TOKEN` 归公开、注释行里的赋值不算配置、短值不许进任何清单（否则到处误命中）。**扫描段 5 条**：保密值原样躺在 `.js` 里必须报、**已公开的令牌在 `.js` 里不许报**（证明白名单真的生效）、同一个值只出现在 `.png` 里不许报（二进制不在扫描范围）、干净文件不许报、样本里根本没有的值不许报（防乱报）。⚠️ 分类段是 09-18 令牌改为「刻意公开」之后加的：**白名单自带一种新失效方式** —— 往 `PUBLIC_BY_DESIGN` 里多写一个名字，常规自检照样全绿，只是从此不再报那个值，只有喂合成样本才看得出来。⚠️ 它的 A/B/C/E 四组是**环境断言** —— 文件在不在、`.gitignore` 有没有把它们挡住、服务起不起得来 —— 自证只覆盖 D 组那两条纯函数判据。另做过一次**真实诱饵验证**：把真令牌写进 `dist/_leak-test.js`，D 组当场翻红，删掉后转绿。这条判据不是恒绿的壳） |
...
**墩儿的接口（2026-09-16 加，8 个）** —— 归在同一个 `server/` 里，见第十三节第 32、36 条；
每个接口内部怎么走、为什么这么切，见 [`docs/duner.md`](./docs/duner.md)：

| 接口 | 说明 |
|---|---|
| `POST /duner/chat` | 说一句话 → 回话 + **指令数组**（前端执行）+ 确认卡 / 澄清选项 + `auditId` |
| `POST /duner/confirm` | 兑换确认令牌（一次性 + 60s TTL）并执行写操作；无效/过期/重用一律 409 |
| `POST /duner/receipt` | 前端执行完的**回执**，按 `auditId` 挂回那次对话 |
| `GET /duner/capabilities` | 能力清单（意图 / 工具 / 词典条数），面板的「你能干什么」 |
| `GET·POST /duner/dict`、`DELETE /duner/dict?term=…` | 术语词典的读与改（**admin**），改完**立即生效**（热读，不是启动时快照）。删除取的是**查询参数** `?term=`，不是路径参数 —— 2026-09-18 订正，原写 `/:term` 与代码不符 |
| `GET /duner/audit` | 最近审计（**admin**）—— 里面存的是所有人的原话 |

⚠️ 前端请求层对墩儿**不做降级**（`src/api/duner.ts` 一律用 `http.post`）：
写接口不许降级的规矩同样适用于对话 —— 降级出一句编造的答复比报错更坏。
后端不可用时面板**如实**显示「墩儿后端未就绪」。
...
| `VITE_CESIUM_ION_TOKEN` | **仓库里公开发布的那一个** | Cesium Ion 令牌，用于在线实景三维。**前端可见**（浏览器里用的东西藏不住），真正的防护是 Ion 后台的**域名白名单**。本仓库 09-18 起刻意把可用令牌随包发布，好让下载者开箱就能试；想换自己的改这一行 |
| `VITE_USE_ONLINE_3D` | **`true`** | 是否用在线实景三维替代程序化场景。⚠️ **国内网络下这是一个确定的 15 秒等待**：该资产的数据托管在 `tile.googleapis.com`（Google），连不上，要等 `ONLINE_3D_TIMEOUT_MS` 到点才退回程序化场景（实测建场景 2005ms → 17851ms）。嫌慢就设 `false`，功能一样完整。**这是知情的默认值，见第十三节第 37 条** |
| `DUNER_LLM_KEY` | 空 | 墩儿的模型密钥。**故意不带 `VITE_` 前缀** —— `VITE_*` 会被内联进前端产物，密钥进去就等于公开。不配也能跑（走离线规则引擎） |
| `DUNER_LLM_MODEL` | `deepseek-v4-flash` | 模型名。必须是支持 function calling 的；`deepseek-v4-pro` 实测返回 200 + 空内容 |
| `DUNER_LLM_BASE_URL` | `https://api.deepseek.com` | OpenAI 兼容接口地址（换本地 Ollama/vLLM 只改这一行） |
| `DUNER_LLM_TIMEOUT_MS` | `8000` | 模型调用超时。**必须明显小于 `src/api/http.ts` 的 15000ms**，否则前端先超时 |
| `DUNER_LLM_ENABLED` | `true` | 一键关掉模型（保留规则引擎），验证「离线也能用」时设 `false` |
| `DUNER_CONFIRM_TTL_MS` | `60000` | 写操作确认令牌的有效期（指导书 4.4 定的是 60 秒） |
| `DUNER_CONFIDENCE` | `0.8` | 规则引擎的置信度门槛，低于它才去问模型。调高 = 更常问模型 |

> `.env` 仍在 `.gitignore` 里、**仍然不要提交** —— 它除了那个**刻意公开**的 Cesium 令牌，
> 还有 `DUNER_LLM_KEY`。公开与保密的边界由 `scripts/check-release.mjs` 的 D 组守着
> （白名单之内**必须**进产物、之外**一个都不许**进），`--self-test` 验的正是这条边界
> 有没有分辨力 —— 白名单自带一种新失效方式：往里多写一个名字，常规自检照样全绿。
> 逐项的完整说明（含每条为什么这么定）在 `.env.example` 里。
...
node scripts/check-all-pages.mjs --self-test      # 同上自证：16 项（分类 6 + 结论行 10，毫秒级）
...
node scripts/check-duner.mjs --self-test          # AI 助手自证：注入缺陷 / 反向期望（先跑这条）
node scripts/check-duner.mjs                      # AI 助手端到端：7 条必过用例 + 295 条测试集 + 17 个工具冒烟
                                                  # （算意图准确率与实体 F1）+ 真浏览器驱动相机/图层/确认卡
                                                  # + 右下角按钮在十个页面上逐页让位（量矩形，不写死偏移）
                                                  # ⚠️ 两种模式**要各跑一次**：自证只证明判据不是恒真，
                                                  #    常规模式才证明真实链路被巡检过（见第五节的教训）
...
| `check-all-pages.mjs`（09-17 复跑） | **10 个页面全部通过、控制台 0 错误**（12 → 7：删掉的 5 个「规划中」占位路由不再巡检；09-14 加回 `/data-admin` 一页 → 8；09-15 拆页把 `/decision` 拆成决策指挥 + 成本管理两页 → 10。这条数在 09-17 之前一直写着 8，是拆页那轮漏改的，本轮按实测订正） |
...
| `check-auth.mjs`（09-14 新增，09-16 修正两条判据后重跑，打 `http://localhost:8787`） | **51/51 项通过，EXIT=0**。接口层：匿名读/写 401、普通用户 5 个写接口**逐个 403**、管理员 201/200 且返回新值、缺必填 400（不是 5xx）、主键非整数 400、删不存在的行 404、备件编码重复 409；界面层：匿名 `#/` → `#/login`、普通用户手敲 `#/data-admin` 被弹回、管理员**新增后刷新页面那条还在**（证明真落库而不是内存态）、界面删除后回到原条数、缺必填时浮层显示后端原文「字段 time 不能为空」且不关闭；隐患按钮**两侧都卡**——普通用户 0 个、管理员 1 个（只卡「普通用户看不到」的话，选择器写错也会绿，所以必须补管理员那一条正对照）。**09-16 修掉两条自己身上的失效**（都不是被检查的代码坏了，是判据本身过期 / 依赖了会被人点掉的数据，见第十三节第 33 条）：① 页签条数原先写死 `=== 4`，「决策工单」那张表加进来之后就一直在红 —— 改成从 `/api/health` 现取台账表数；② 隐患按钮那条正对照要求库里**至少有一条非终态隐患**，而演示时被一条条点成了「已处置」，于是它红在一个跟页面无关的理由上 —— 改成先把 #3 复位成种子里的「处置中」（幂等，顺带让该页演示恢复可用） |
| `check-auth.mjs --self-test`（09-14 新增） | **8/8 项通过** |
| `check-duner.mjs`（09-16~17 新增、**09-17 加右下角按钮与十页让位后重跑**；打 `npm run serve` 的 8787，`MINE_DB=:memory:` 与磁盘库两种起法都跑过） | **105 项，失败 0**。甲段 7 条必过用例逐条过（含「删除所有数据」逐字回绝且 `commands` 为空、注入句不换人格、应急句免确认直达），**另加⑧ 本轮补的四条：点名问位移必须回那个点的数字（26.4mm）、方位词「北帮」真下发一条 `map.flyTo`、「不是这个，我说的是东帮」改飞东帮（原来答"这句我没听懂"）、光杆「不是这个」反问后打字「东帮」能接上**；二段确认闸门 22 项（未点确认前台账里没有、兑换后能从**另一条路由**读回来、同一令牌再用 409、无令牌 400、非管理员拿不到确认卡、审计四段闭合）；三段能力清单与词典热更（改词条 → 审计里的 `normalized` 跟着变 → 删掉又变回来）、403/401；四段词表与前端源码漂移比对；五段测试集 **295 条**（生成 244 + 手写 51）：**意图准确率 100%（生成组 244/244、手写组 51/51）、实体 F1 100%（TP 273 = 213 + 60）**，两组**分开计分**（合在一起算的话，手写组烂掉看不出来）；六段真浏览器：右下角按钮确实挂在 1920×1080 缩放容器里（**1440×900 下 66px → 49.5px**、1920×1080 下 66px，等比缩放）、落在右下角那条带上（距底 16px）、**十个页面逐页扫描按钮的落位**（每页等到让位值稳定再量矩形，断言不压任何宽度 < 1/3 画布的元素——右栏、列表、页签，且不出画面；实测 16/804/428/16/964/1137/428/16/16/16）、点按钮开对话框、输入框只在开着时存在、点 ✕ 关掉、开合都不移动按钮，然后三句话把相机真的飞了（443m / 3559m / 同一地名两次落点差 0.0m）、图层隔离后读**页面自己的状态**与**实体的 show** 都对、写操作弹确认卡且**点之前一次 `/duner/confirm` 请求都没发**、点之后用另一条路由读回那张单；七段**工具冒烟**（这一条是被一个真缺陷逼出来的）：把全部 **17 个**工具（只读 12 / 写 5）**拿空参数各真跑一遍**，断言不许回「执行失败」，**再拿「最小合法参数」跑 18 条组合**，断言既不许崩、也不许反问（参数合法时反问就是 bug）—— 这类「用了没引入」的缺陷从甲/乙/五段的每一条底下钻过去（不抛异常、不是 5xx、纯函数段压根不走 `execute()`、控制台干净），只有真跑一下才看得见 |
| `check-duner.mjs --self-test`（09-16~17 新增、09-17 补 T6/T7） | **120 项，失败 0**（T1/T2/T4 在真实链路上注入缺陷、T3 喂反向期望、T5 现注册一个必崩工具验证七段的判据不是空壳、**T6 注册一个"只有给了参数才崩"的工具：先证明"空参数"那条确实抓不到它，再证明新加的"最小合法参数"那条当场翻红**、**T7 手动把 `--duner-right` 扳回 16px：十页扫描同一把量法当场抓到 `nav.twin__tabs（400×52）` 被压住，点一下按钮还原后又什么都不压**；两个坑是上一轮**自证自己抓出来的**：空函数没回调 `complete` 导致前端 `busy` 一直挂着、注入那句话的目的地与相机当前所在处**恰好相同**使得断言恒真） |
...
| `check-release.mjs`（09-18 新增） | **21 项全过；`--self-test` 10 项全过**（19 项那一版曾在 C2「产物不比源码旧」上红过一次 —— 当轮 `dist/` 还没提交，打印出来是「产物 从未提交」，正是它要抓的东西，提交后转绿）。D 组现在是「白名单之内必须进、之外必须不进」的双向判据。诱饵验证做过一次：真令牌写进 `dist/_leak-test.js` ⇒ D 组当场翻红，删除后转绿 |
...
**方式一（推荐）：一个进程同时托管 `dist/` 与 `/api`。** 零第三方依赖，
连构建都省了 —— `dist/` 与种子库都随仓库分发：

```bash
node server/index.mjs            # 默认 8787；`--port` / `--db` 可覆盖
```

**方式二：只要静态页**（没有 `/api`，登录会失败，只能看到登录页）：
...
30. **数据入库 + 数据管理 + 登录/角色权限（2026-09-14 加）** ——
    ...
    **实现要点**（细节见第十节）：`server/` 手写 `node:http` + Node 24 内置
    ...
    库文件分两份：`mine.seed.db`（种子基线，只读，**随仓库分发** —— 没装依赖的人
    跑不了 `db:seed`，少了它下载者打开就是四张空表）与 `mine.db`（工作库，
    运行时一直在写，**必须 gitignore** —— 二进制文件 diff 不出内容，容易顺手把
    一堆审计记录一起提交）。两份里的口令都是 `scrypt` + 每用户随机 salt 哈希，
    不是明文；种子库里只有登录页上写着的那两个演示账号。
...
32. **墩儿（AI 助手）：一句话驱动三维与业务系统（2026-09-16~17 加）** ——
    按 `参考/AI助手指导书_墩儿_自然语言驱动智慧矿山平台_Java后端版.md` 实现：
    用户在右下角的对话框里说一句话，后端判意图、抽实体、过权限、把**指令**随响应体发回前端，
    前端执行（相机 / 图层 / 面板）并回执，一切落审计。

    ⚠️ **它不在《项目文档.docx》的条目内**（见第十四节）。docx 是功能范围的唯一基准，
    这一项是指导书带来的**新增能力**，如实记在这里而不是混进「docx 落地情况」里。

    **技术选型与指导书不同，是用户拍板的**：指导书写的是 Java/Spring Boot + Spring AI，
    本机没有 JDK 17+（只有 JDK 1.7 / JRE 1.8）、没有 Maven，装了才能跑；
    用户选择**不装**，把指导书的**架构**（后端中心化 NLU / 决策 / 权限 / 确认 / 审计，
    前端只执行视图指令并回执）照搬到本仓库既有的手写 `node:http` + 内置 `node:sqlite` 后端上，
    **零新增依赖**。另外指导书设想的 WebSocket 下发改成**随 `/api/duner/chat` 响应体返回**
    —— 每条指令都是对用户某句话的应答，不需要后端主动推送；且本项目的 WS 是
    **刻意默认关闭**的（握手失败会在控制台留 error，破坏「0 控制台错误」这条验收线）。

    **四条结构性约束（不靠提示词，靠代码）**：

    | 约束 | 落实方式 |
    |---|---|
    | 未经确认的写操作 = 0 | 注册表里 `readOnly: false` 的工具**一律**经 `ConfirmManager` 发确认卡（一次性令牌 + 60s TTL），与意图来自规则还是模型无关；`check-duner.mjs` 在浏览器里数 `/duner/confirm` 的请求数——**点之前必须是 0** |
    | 不许越权 | `role === 'admin'` 由后端逐个写接口校验，普通用户拿到的是 `DENY_WRITE`，**拿不到确认卡**（也就没有「确认一下就能写」这条路） |
    | 一切可审计 | 一次对话落一行 `audit_log`（原话 / 归一化后的文本 / 意图 / 槽位 / 工具 / 结果 / 耗时）；写操作**单独一行** `duner.write`；前端回执按 `auditId` 挂回那次对话 |
    | 离线可用 | 规则引擎 + 术语词典是**主路径**，模型只是置信度不足时的兜底；`DUNER_LLM_ENABLED=false` 时导航 / 图层 / 查询 100% 可用 |

    **模型**：DeepSeek `deepseek-v4-flash`（OpenAI 兼容接口，function calling）。
    实测三条：① **工具名不能带点号**（`map.flyTo` → HTTP 400），线上名一律下划线、
    点号只作内部逻辑名；② `deepseek-v4-pro` 返回 HTTP 200 但 content 为空，
    所以用 flash；③ 中文**不能用 `curl -d` 发**（按 GBK 出去、服务端乱码），要发就用 node `fetch`。
    密钥走 `DUNER_LLM_KEY` —— **故意不带 `VITE_` 前缀**，`VITE_*` 会被内联进前端产物。

    **指标（第 32 条那一轮实测，`check-duner.mjs` 两组分开计分）**：意图准确率 100%（生成组 232/232、
    手写组 43/43）、实体 F1 100%（TP 250）、测试集 275 条 ≥ 200 条、
    未经确认的写操作 0 次。指导书要求的是 ≥95% / ≥92%，这里是**实测值**不是目标值。
    **这套数不是最终态**：测试集在 09-17 那一轮扩到 **295 条**（生成 244 + 手写 51，
    补了点名问询 / 方位词 / 纠错 / 打字确认告警四类），两组仍各 100%、
    实体 F1 100%（TP 273 = 213 + 60）—— 两轮的分母不同，**别把这两组数混着引用**，
    见第 34 条与第十一节。

    **本轮被检查脚本抓出来的四处真缺陷**（都是「不报错、只是听不懂/答错」的那种）：

    - **「主运输道路上翻车了」认不出来**：`emergencyKind` 认得出 `运输/翻车/车辆/追尾/撞`
      是运输事故，而应急**触发词表**里一个事故词都没有 —— 那套分类是**永远走不到的死代码**。
      矿上真翻了车，说这句话只会落到模型那条路上去；模型不可用时就是「我没听懂」。
    - **「派谁去救」凭空认定了事故类型**：`emergencyKind` 认不出时兜底成 `landslide`，
      于是回话是「已匹配《边坡滑坡事故专项应急预案》」——用户全程没提滑坡。
      改成认不出就**不带 kind 槽**，交由 `plan.match` 反问「是哪一类事故？」。
      这与 `extractDomains` 认不出领域时返回空数组是**同一条规矩**（把握不足就反问）。
      顺带把反问选项从简称（「水害隐患」）换成**预案全名**（「坑底积水（透水）专项应急预案」）——
      简称里一个触发词都没有，用户点一下会被再问一遍。
    - **「皮带二」被词典啃掉一块**：`皮带廊: ['皮带', …]` 的同义词替换把真实设备名
      `皮带二` 改成了「皮带廊二」，查无此设备。加了一道与 `insideKnownPlace` 同源的闸门：
      命中后面**紧跟序号**时不许替换（`normalize.mjs` 的 `followedByOrdinal`）。
    - **`data.query` 里 `clarifyResult` 用了没引入**（第 4 处，**前三处修完之后才发现的**）：
      说「5号采区品位下降了多少」这类认不出领域的话，回来的是
      「工具 data.query 执行失败：clarifyResult is not defined」。它躲过了当时**全部**判据 ——
      不抛异常（`runTool` 把异常兜成了 `errorResult`，界面上是一句"看起来正常的回话"）、
      不是 5xx、五段是纯函数段压根不走 `execute()`、甲段 7 条必过用例说的话术全都命中规则分支、
      控制台干净。**是手工点了一次才发现的**，发现之后给 `check-duner.mjs` 补了第七段
      「工具冒烟」：把全部 17 个工具**拿空参数各真跑一遍**，断言不许回「执行失败」；
      并在自证段加了 T5（现注册一个必崩的工具，那段判据必须发现它）与一条反例
      （参数不齐时那句「要找谁？」是**正常**的 error，不许被算成崩）。
      实测把 `clarifyResult` 的引入再拿掉一次，这段当场报 `data.query → 执行失败：clarifyResult is not defined`
      —— 判据对**这个真缺陷**是有效的，不只是对合成注入有效。

    **如实记账的取舍**：

    - 「安全监测点」「成本」**没有图层开关**（不在任何一页的可操作图层表里）——
      用户说「打开安全监测点图层」时，墩儿如实回该页没有这个图层，不假装成功。
      `check-duner.mjs` 把这两个名字列进「无开关名单」白名单，**名单之外**出现孤儿就判红。
    - 应急句「着火了」**不套任何预案**：本矿四份预案里没有消防专项，凑一份最接近的
      （比如运输事故）会给错行动指令，所以如实回「本矿没有匹配这一类」并列出有的四类。
    - T1/T2 那两处注入**是自证抓出来的**（空函数没回调 `complete` 让前端 `busy` 一直挂着、
      注入那句话的目的地恰好等于当前相机所在处，使断言恒真），不是设计时就想到的。

33. **`check-auth.mjs` 里两条失效的判据（2026-09-16 修）** ——
    做第 32 条时按规矩把回归脚本重跑一遍，`check-auth.mjs` 报 48/50。
    两条都不是**被检查的代码**坏了，是**判据自己**坏了 —— 这类失效最难发现，
    因为它红的时候看起来像功能坏了，绿的时候看起来像功能好了：

    - **写死的数字过期了**：`页签条数与库一致（4 张表）` 断言 `=== 4`，
      而「决策工单」那张表在 09-15 拆页那一轮（第 31 条）就进了台账，
      页面一直是 5 个页签。改成从 `/api/health` 的 `tables` **现取**表数再比
      —— 加表它跟着走，**真少一个页签**（`v-for` 改坏、`RESOURCES` 少一项）
      才会翻红。另加 `> 0` 防它退化成恒真（health 取不到时两边都是 0）。
    - **判据依赖了一份会被点掉的数据**：隐患按钮那条正对照要求库里**至少有一条
      非终态隐患**（按钮只在非终态行上渲染），而演示时那四条被一条条点成了「已处置」，
      于是它红在一个**跟页面无关的理由**上——数据用完了。这不是放宽断言能解决的
      （放宽成 `>= 0` 就是恒真），而是要把**前提摆好**：先 `PUT` 把 #3 复位成
      `src/mock/emergency.ts` 里种子的「处置中」（幂等），顺带让应急救援页的演示恢复可用。

    修完重跑：**51/51 通过**（多出来的一条就是那句「复位成功」，它是后一条的前提；
    前提自己也要有断言，否则前提没成立时后一条会红在一个误导性的理由上）。
    `--self-test` 仍 **8/8**。

34. **墩儿的第二批：位移答非所问 / 纠错听不懂 / 方位词自相矛盾 / 打字回答断链（2026-09-17 上午修，
    比第 35 条早，按写入顺序排在前面）** ——
    这一轮起点不是"再想想能加什么"，而是**拿真后端把话说一遍**。
    实测出四类「不报错、只是听不懂 / 答错」的缺陷 —— 正是本模块唯一一类用户看不出来的故障。
    ⚠️ 其中第一条**把判据自己带坏了**，值得单记。

    - **「北帮3号台阶现在位移多少」答非所问**。现象：回话是「重大风险源 2 项，存在风险 5 项，
      隐患闭环 48/46/41/39…」，**一个位移数字都没有**（数据就在 `src/mock/digitalTwin.ts` 里，
      SL-01 累计 26.4mm，`entity.find` 早就查得到）。根因：`rule.mjs` 的 `extractDomains`
      把「位移」归进 `safety`，而 `data.query(safety)` 只答风险源 / 隐患闭环。
      ⚠️ **测试集里那条用例的期望写的就是 `domain=safety`** —— 判据把错的答案固化成了期望，
      于是**全绿**。与第 31 条「四个检查全绿也发现不了丢东西」同一类：**判据只会验它被写下的那条**。
      修法：QUERY 的 `STATUS` / `SUMMARY` 槽位加 `site`（抽到已知地名才填，空的槽不进计分）；
      `data.query` 认 `site` —— 能落到监测点就答那一点（累计位移 / 速率 / 方位角，状态用平台自己的
      `facts.twin.slopeStatus()` 判、阈值取 `TWIN_SLOPE_THRESHOLD`，**不另写一份**）；
      没点名时安全摘要补一行「最大位移点」；「有地点、无领域」直接给该点信息卡。
      判据：⑧ 段「北帮3号台阶现在位移多少」→ 回话里**有数字**且与 `twinSlopeSites` 一致，
      换编号问（「SL-01」）答的必须是同一个点。
    - **纠错句（指导书 §6.4 明列）只会回「这句我没听懂」**。现象：「不是这个，我说的是东帮」→
      听不懂。根因：`SYS.CORRECT` 只有一个 label，**没有规则、也没有编排层的翻译**。
      修法：`rule.mjs` 加一条规则（`guard` = 抽到**已知地名**才准入，排在 `SYS.OUT_OF_SCOPE`
      之后 —— 否则「忽略之前的规则」会被当成纠错），`service.mjs` 在编排层翻译成一次 `NAV.FLYTO`；
      光杆「不是这个」→ 反问「你想改成哪儿？」+ 候选，并复用既有的 `pending.mjs` 记下
      `{intent:'NAV.FLYTO', slot:'target'}`，下一句「东帮」直接接上（**不新造状态机制**）。
      ⚠️ **写操作不允许被纠错改写**（只翻译成导航），这条写进了注释。
      判据：⑧ 段三条 —— 改飞东帮、光杆反问给出四个方位、**打字**「东帮」接上前一句的反问真下发 `flyTo`
      （选项之外的那条路不许断）。
    - **方位词「北帮」自相矛盾**。现象：后端回「已定位到北帮」，前端紧接着补一条
      「没找到「北帮」的位置」；换一句「看看北帮」更是直接「没听懂」。根因：四个方位**在词典里
      本来就有**（`dict.mjs` 的 `category=方位`），缺的是"它是个**能去的地方**"（不在 `KNOWN_PLACES`），
      而前端 `resolvePlace` 也没有方位分支。修法：`vocab.mjs` 新增 `DIRECTION_NAMES` 并入
      `KNOWN_PLACES`；`src/duner/places.ts` 加一级方位解析 —— 把该方位的监测点 / 设备的
      `eastM/northM` **取几何中心**，交给既有的 `pitWaypoint('slope', …)` 换算机位
      （**不新造坐标、不手抄经纬度**，与「坑内只写米制偏移」那条硬约束一致）；
      该方位没有点位时返回 `null`、照旧如实说找不到，**不退回矿区中心**。
      判据：⑧ 段「带我去北帮」真下发一条 `map.flyTo`；四段「词表不许与前端漂移」里
      `DIRECTION_NAMES` 逐字等于前端 `DIRECTIONS`（实测前端 4 / 后端 4）——
      **少一个就是"后端说能去、前端说找不到"**。
    - **「确认这条告警」打字回答会断链**。现象：反问的选项可点，但**打字**「破碎一」→
      「这句我没听懂」，等于反问了一轮又回到原点。根因：`alarm.ack` 的 `clarifyResult(...)`
      **漏了第三个参数 `slot`** —— 同文件其它三处反问（`order.create` / `report.export` /
      `decision.adopt`）都带了。修法：补 `slot:'device'`。
      判据：⑧ 段「确认这条告警」→ 反问并给出四条可点选项（点选项那条路不许回归）。

    **同一条根因逼出来的检查补强**：第七段「工具冒烟」原来拿**空参数**跑 17 个工具，走的全是
    「参数不齐」分支 —— 它抓到了第 32 条那个 `clarifyResult` 用了没引入，
    但同类缺陷若落在**成功分支**（比如领域摘要里用了没引入的函数），照样漏。
    所以加了一张「每个工具的最小合法参数」表（**18 条参数组合**）逐个真跑，断言**既不是 error
    也不是 clarify**（参数合法时反问就是 bug）；自证段补 **T6**：临时注册一个"只有给了参数才崩"的
    工具 —— 先证明「空参数不许崩」那条**确实抓不到它**（这段判据原本是瞎的），
    再证明新的「合法参数」判据**当场翻红**（实测 `duner_selftest_throw_with_args → 执行失败`）。

    **测试集随之扩容**：275 → **295 条**（生成 244 + 手写 51），补了点名问询 / 方位词 / 纠错 /
    打字确认告警四类；**并且把那条把错答案固化下来的期望改掉了**（这才是它真正的价值 ——
    不然新一轮的修复会被一条旧的错误期望判红）。两组仍各 100%、实体 F1 100%（TP 273）。

    **如实记账的取舍**：

    - **语音输入没做**（用户当时选的就是"不做，如实记账"）。参考的那版与指导书里都有，
      本项目**不实现**：Web Speech API 依赖 Google 在线识别，与本项目「离线可用」
      （`DUNER_LLM_ENABLED=false` 时导航 / 图层 / 查询 100% 可用）相冲突，
      握手失败还会在控制台留 error、破坏「0 控制台错误」这条验收线。入口只留打字与点选。
    - **光杆「不是这个」能走多远，只测到"反问 + 接一句「东帮」"**。它给的候选是四个方位，
      接一句能解析的地名才真的改飞；**回一句解析不出的地名会怎样，本轮没量，不替它打包票**。
      没有上下文时它不猜 —— 这与「把握不足就反问」是同一条规矩。

35. **墩儿的入口从底部命令栏改成右下角按钮：三个"量了才知道"的坑 + 一处读旧值（2026-09-17 改）** ——
    按用户要求把入口改成"右下角一个按钮，点开才是对话框"（形态参考 `D:\Zibo-SmartTransportation-WebGIS-main3`
    的 AI 助手：贴边展开、无遮罩、按钮保持可见并变激活态、底部快捷问句只回填不发送；
    配色不走它那套浅色，仍用本项目的深色 token）。原来是一条底部居中的 640px 命令栏。
    改完当场量十个页面，量出**两件事：一件布局的、一件判据的**。

    **布局侧的现象**：数字孪生页按钮落在距右 16px —— **正好压住最右那个页签**
    （`nav.twin__tabs` 400×52，页签是**可点控件**）。这正是 `check-panel-overflow.mjs`
    立规矩时要抓的那类缺陷：「DOM 里那块面板和它的 canvas 都在，计数完全正常，
    只有量矩形（或者肉眼看截图）才发现得了」。

    **根因**：**没有一条规则能让按钮"知道"右边有什么**。按页面配一张偏移表 = 把十个页面的
    布局复制进墩儿里，页面一改布局，这张表**不会报错**、只会把按钮安静地压回某个面板上
    （与第 32 条「坐标不进后端」同一个理由：副本会漂移，且漂移了没人发现）。
    所以改成**当场量**：`src/duner/corner.ts` 从右下角起，谁在按钮那条横带上就往左让过谁，
    一直让到没人。修的过程里出了三个坑，**全是量了才知道的**：

    - **① 让一次不够**：安全/设备/应急三页的底部是**两块并排**的面板（右栏一块、紧挨着左边
      还有一块 360~460 的），只躲一次正好落在第二块的右端上 —— 看上去躲开了，压住的其实是
      隔壁那块。改成迭代（上限 8 次，实测最多用到 4 次，在应急救援页）。
    - **② 让位不能让出画面**：设备管理、数据管理两页的底部整行铺满，一味往左让会一路算到
      1916（66 的按钮落到 x=−62，等于消失）。加了「再往左就顶出画面 ⇒ 停在原地」的护栏，
      代价就是下面表里那三处**明知而接受**的压角。
    - **③ 阈值定得太宽，反而让按钮压住了窄面板**（这条最值得记）：「候选面板的宽度上限」
      第一版写成了 9/10（超过它的算铺底、不用躲），于是安全管理页那块 1440 宽的地图容器
      被当成候选，迭代算出的落点在画面外、被护栏截住，按钮停在 492 —— **正好压在底下那块
      360 宽的隐患面板上**，比改之前更糟。线改成 2/3（1920 下 1280px）后回到 804、干净。
      教训：**宽面板该不该躲，量的是宽度，不是"躲不躲得开"**。

    **判据侧的现象**：决策指挥页第一轮扫描量到 428、第二轮量到 16。同一条规则、同一页，
    两次读数不同，看起来像规则不稳。用时间线转储量了现场：读数是**上一页的旧值** ——
    路由切过去 4 秒后 `--duner-right` 还是数字孪生页的 428，而新页的底部面板行已经就位，
    于是按钮右缘压住最右那块面板 5px。**根因不在规则，在测量时机**：这台机器没有 GPU 直通
    （SwiftShader 软渲，实测 0.2–0.7 fps、一帧 2–3 秒），`requestAnimationFrame` 与
    `setTimeout` 会被拖后好几秒，而 `MutationObserver` 的回调是**微任务** —— 当前任务一结束
    就跑，不会被长渲染饿死。修法：路由变化时重挂 observer、**在微任务里同步量**，
    另配一个 400ms 的兜底重测（只测一次不够：新页的 DOM 是分几批进来的）。
    ⚠️ 已记进 `DunerDock.vue` 的注释 —— 这类「读到上一页的值」不会报任何错。

    **让位后的实测落位（1920×1080，两轮扫描一致；`check-duner.mjs` 逐页断言）**：
    ...
    | 应急救援 | 1137 | 右栏（380）+ 795 起那块 713 的面板（底下并着两块 336） | **同排左边那块 713 面板的右端，压住 62px** |
    | 设备管理 | 964 | 右栏（460）+ 并排的 460 面板 | **936px 气泡面板的右下角，压住 62px** |
    | 数据管理 | 16 | 没人挡 | **1888px 表格面板的一角** |

    断言的是**窄的那一类**：十页里没有任何一页压住宽度 < 1/3 画布的元素（右栏 / 列表 / 页签
    —— 真会被点、会被看的东西）。那三页压住宽面板一角，原因是底部整行铺满、**中间没有第二个
    66px 的落点**（应急救援页实测：底部那行 66..779 与 795..1508 两块面板、右边再顶着 380 的
    右栏，中间只剩 16px 的缝；按钮 x 717..783 搭在左边那块 x 66..779 的右端上），**明知而接受**，
    记在这张表里而不是假装没有。⚠️ **这条实测推翻过自己的注释**：`corner.ts` 的表原来把
    应急救援页写成"只有底图"，是 `check-duner.mjs` 的运行时报出它压着 713 的面板 ——
    量了一遍确认**判据对、注释错**，改的是注释。

    **判据的盲区也如实记**：宽度超过 2/3 画布的铺底元素（数据管理那块 1888 的表格、
    应急救援那块 1442 的底行容器）**两边都不算**，逐页扫描不覆盖它们。数据管理页按钮压的就是
    那块表格的最右下角一格 —— 它是取舍，不是漏检，但没有判据看着它。

    **判据**：`check-duner.mjs` 第六段逐页扫描 —— 每页 `goto` → **等到让位值稳定**
    （每 400ms 采样、连续两次相同才算稳，最多 8s）→ 量矩形 → 断言不压窄元素且不出画面；
    自证段补 **T7**：手动把 `--duner-right` 扳回 `16px`，同一把量法必须当场看见按钮压住页签
    （实测抓到 `nav.twin__tabs（400×52）；button.twin__tab（122×34）`），点一下按钮还原之后
    又必须什么都不压。**没有 T7 的话，逐页扫描那段可能是个恒绿的壳** —— 与第 32 条
    「一个从没见过它变红的检查不算证据」是同一条规矩。实测：常规 **105 项 / 0 失败**（原 76），
    自证 **120 项 / 0 失败**（原 87）；`check-all-pages.mjs` 十页全过、控制台 0 错误。

    **如实记账的取舍**：

    - **语音输入没做**（理由与取舍见第 34 条；参考的那版有这个入口，本项目刻意不跟）。
    - **按钮压在宽面板上的那三处**是取舍不是遗漏，算式与代价写在上表下面。
    - **本轮把日期订正回实测**：第 32/33 条与本节几处原写"2026-09-19"，按文件 mtime 实为
      **09-16~17**（`server/duner/*` 09-16 18:18–20:05 与 09-17 10:56–11:08、
      `check-auth.mjs` 09-16 20:41、指导书 09-16 15:22）—— 与第 33 条同一类失效：
      **写死的数字/日期会漂**，能当场量的就当场量。

36. **同步前的收尾：按开发者视角补文档与展示图，顺带修掉「TTL 文案写死」（2026-09-18）** ——
    这一轮不加功能，只把已经做完的东西按**读代码的人**的标准补齐：两张展示图、
    一份实现说明（[`docs/duner.md`](./docs/duner.md)）、提交信息按模块写清。
    过程中抓到一个真缺陷与两处口径不符。

    - **确认卡的「60 秒」是写死的，而 TTL 是配置项**。`DUNER_CONFIRM_TTL_MS` 改过之后，
      卡片倒计时按配置走（前端用后端给的 `expiresInMs`），**回话却还说 60 秒**
      （`service.mjs` 拼回话那句、`confirm.mjs` 的过期文案各写死一处）—— 两边对不上，
      而**没有任何报错**。暴露它的方式是**拍展示图**：软渲染下一张全屏截图要 60~90 秒，
      第一版图里那张卡从「60s 内有效」掉到了「0s」，于是把 TTL 临时配成 180s 重拍 ——
      这一配就把那句假话配出来了（图注里如实写着 172s 的来由）。
      修法：两处都从 `CONFIRM_TTL_MS` 现算。**与第 33、35 条同族：写死的数字会漂。**
    - **README 两处口径与代码不符**（同日订正）：① 第十节写「duner 接口 7 个」，
      `FIXED_ROUTES` 实为 **8 条**（chat / confirm / receipt / capabilities / dict×3 / audit）；
      ② 词典删除写成 `DELETE /duner/dict/:term`，代码取的是 `ctx.query.term`，
      巡检脚本一直用 `?term=` —— 文档与代码各自都"看着挺合理"，只有对着读才发现不一致。
    - **新增**：`scripts/make-duner-shots.mjs`（两张展示图 + **自带两条判据**：三维区不能是空地球；
      截确认卡那一步**不许发出兑换请求、库里不许多出单子**）、`docs/duner.md`
      （模块地图 / 一次请求的 16 步链路 / 规则优先的三条实测硬约束 / 写操作的五道结构 /
      前端三件事 / 判据怎么跑 / 已知限制）。README 首屏补了功能行简介与那两张图。
    - **最终验证**：修完 TTL 文案后重跑，`check-duner.mjs` 常规 **105 项 / 0 失败**、
      `--self-test` **120 项 / 0 失败**（两种模式分开跑，见该脚本头部的规矩）；
      测试集 295 条：意图准确率 100%、实体 F1 100%。**未重跑其余巡检**
      —— 本轮只动了 `server/duner/` 的两句文案与文档，未碰页面与三维。

37. **默认打开在线实景三维：令牌随包发布，以及它真实的代价（2026-09-18）** ——
    用户要求「除了 DeepSeek 的 api，别的都传上去」。于是 `VITE_CESIUM_ION_TOKEN`
    从「必须藏好」改成**刻意公开发布**（它是浏览器里用的令牌，真正的防护是 Ion
    后台的域名白名单，见 `.env.example`）。这一轮把边界、判据与代价一起理顺。

    - **先说没有的东西**：用户提到 mapbox，但**全仓库搜 `mapbox` 零命中** ——
      本项目没有 Mapbox 依赖或令牌，底图走的是 `public/map-tiles` 的离线瓦片。
      Cesium 的库与资源（`public/cesium/` 389 个文件 + 2724 张瓦片）上一轮已入库。
      所以这一轮实际动的只有**一个令牌变量**。
    - **令牌的边界变了，判据必须跟着变**。`check-release.mjs` 原写「`.env` 的值
      一个都不许进 dist」，按新决定它会**故意判红**。改法是引入白名单
      `PUBLIC_BY_DESIGN = { VITE_CESIUM_ION_TOKEN }`，拆成四条：
      **D1** 白名单之外的保密值一个都不许进（`DUNER_LLM_KEY` 是重点）；
      **D2 公开令牌必须在产物里** —— 正向对照，因为缺了它**不会报错**：
      在线三维会静默退回程序化场景，功能"看着还是好的"；
      **D3** `.env.example` 里也得带着它，否则别人 `cp` 之后重建会静默丢掉，
      而那时 D2 在**对方机器上**会因为 `.env` 里没有长值而跳过、**照样判绿**；
      **D4** `.env` 本身仍不许入库（它还有 `DUNER_LLM_KEY`）。
      自证从 4 条扩到 **10 条**，新增的一段专门验**分类**：允许清单写错了
      （比如手滑把 `DUNER_LLM_KEY` 也放进去），常规自检照样全绿，
      只有喂合成样本才看得出来 —— 这是白名单自带的新失效方式。
    - **⚠️ 实测：这个「在线」在国内根本连不上，代价是每个三维页白等 15 秒。**
      打开 `VITE_USE_ONLINE_3D` 重建后，`check-scene-load` 报建场景
      **2005ms → 17851ms**，多出来的正好是 `ONLINE_3D_TIMEOUT_MS`。逐跳查：
      `api.cesium.com` 通（405 / 0.77s）、`assets.ion.cesium.com` 通（401 / 0.33s）、
      **Ion 的 endpoint 也返回 200** —— 但响应里写着根瓦片在
      **`tile.googleapis.com`**，而那是 Google 的服务：本机 `curl` 对它
      **HTTP 000、20 秒超时、连接耗时 0.000000s**（连 TCP 都建不起来）。
      一句话：**令牌是好的、Ion 是通的、数据在 Google 那儿。**
      把这个如实告诉用户、并给出「回退默认关」与「超时改 3s」两个选项之后，
      用户的选择是**仍然默认开、并接受这十几秒** —— 所以它是**知情的默认值**，
      不是疏忽。`.env.example` 里那段注释就是照这个事实写的。
    - **连带修的两处判据**（都是被那 15 秒拽出来的，都属于「红得像功能坏了」）：
      ① `check-scene-load.mjs` 的 `canvas.hover()` **卡满 30 秒超时**，报错停在
      `waiting for element to be visible and stable`。这是第 31 条那个可操作性检查
      被渲染循环饿死的老坑，只是这一处**没走 `lib/click.mjs`**（全仓库仅此一处裸
      `.hover()`）；修法是改用 `page.mouse.move()` —— 这里要的只是"把指针挪到画布上，
      好让 wheel 滚画布而不是滚页面"，`hover()` 附带的检查纯属多余。
      ② 同一脚本 A3 的耗时上限 20000ms 只剩 2.1 秒余量，放宽到 30000，并在注释里
      写明**这不是为了让哪一次变绿**：A3 抓的是「永远建不完」（96 秒仍是 0 个实体
      那种无限过程），30 秒和 20 秒一样抓得到；真正量预热有没有生效的
      A2（新取瓦片 0 张）**一个字没动**。
    - **`check-all-pages.mjs` 补了「错误明细」**：它报「1/10 个页面有 1 条错误」，
      却**只给数字不给原文**，排障等于从零开始 —— 这是第 32 条那类「不报错的错」的
      镜像：**报错但不说是谁**。现在失败行会带上前 5 条原文。
    - **那条间歇性红查清了：就是上面那条到 `tile.googleapis.com` 的超时。**
      它**是真的，但不是缺陷** —— 是上面那个「默认打开在线三维」的必然产物。
      三次运行的现象（**每次 1 条、每次页面都不同、探针复现不到**）全部由同一个
      机制解释：该请求 Chromium 一直重试到 **87.5 秒**才放弃，并打出一行
      `Failed to load resource: net::ERR_CONNECTION_TIMED_OUT`。它不含 `404`
      也不含 `/api/`，于是被判成真错误；而巡检在 22 秒统计窗口之后还要**截图**
      （软渲染 60~120 秒），**那个长窗口正是它落进来的地方** ——
      前两次探针只开 22s / 50s 窗口，所以读到 0 条；「每次页面不同」也由此解释：
      **谁的截图慢，谁就挨上**。把探针窗口拉到 180 秒，一次就复现：

      ```
       16434ms  [requestfailed] net::ERR_ABORTED  http://localhost:8787/models/mine-tileset/tileset.json
       87532ms  [requestfailed] net::ERR_CONNECTION_TIMED_OUT  https://tile.googleapis.com/v1/3dtiles/root.json?key=…
       87532ms  [console.error] Failed to load resource: net::ERR_CONNECTION_TIMED_OUT
      ```

      （顺带记：那条 `ERR_ABORTED` 是本地模型 tileset 在在线三维超时退回时被取消，
      属于 `requestfailed` 而**不是** console error，不计入错误数。）
    - **修法：给这条错误自己的口径，但收窄、且要证据。** 新增 `外网不可达` 列，
      与 `接口未就绪` 同级 —— **报出来，但不判失败**（`挑问题` 不看它）。
      关键是它**不能**退化成「网络错误一律不算」：豁免要求本页**真的**有一条
      发往该主机的 `requestfailed`，且错误文本要对得上（`ERR_CONNECTION_RESET`
      照样算真错误）。分类因此**从事件监听里挪到本页跑完之后**做 ——
      `requestfailed` 与 `console` 是两个独立 CDP 事件、到达顺序没有保证，
      当场分等于抛硬币。自证从 9 条扩到 **16 条**，其中带 ★ 的两条专门证明
      这条豁免**收得够窄**：「没有失败记录时同样的文本不许豁免」
      「豁免只吃那一条，同页其它真错误照报」。
      另外分类也**必须晚于截图**，早一点就漏掉那条 87 秒才到的错误。
    - **最终验证**：`check-all-pages` **10/10 全过、退出码 0** ——
      而且**修复确实生效了、不是侥幸**：同一次运行里**决策指挥**
      `错误=0 / 外网不可达=1`，正是第二轮判红的那一页、那条错误。
      换在修复前，这一轮会打印 `✗ 1/10 个页面有问题：决策指挥：1 条错误`。
      `check-release.mjs` **21 项全过**、`--self-test` **10 项全过**；
      `check-scene-load` **10 项全过**（建场景 18030ms）。**未重跑其余巡检**
      —— 本轮只动了构建配置（`.env` / `.env.example`）、`dist/` 与上述三个脚本。
...
> ⚠️ **一件要单独记的事：墩儿（AI 助手）不在 docx 的条目内。**
> 它的依据是 `参考/AI助手指导书_墩儿_…….md`（2026-09-16 加入），
> 六章 docx 需求里**没有**「自然语言指挥」这一项。按上面那张表的口径，
> 「代码有而文档没要求的**删掉**」—— 但这次**没删**，理由是用户明确要求做它，
> 且它不改变任何既有页面/接口的行为（右下角按钮与对话框是新增的浮层，指令执行走既有的
> 相机与图层句柄；写操作仍走既有的工单接口与权限判定）。
> 记在这里是为了不让它**冒充**成 docx 的落地项 —— 本轮所有
> 「docx 落地情况」表格一个字都没动。详见第十三节第 32 条。
...
- **需要登录才能进入系统**（`admin` / `admin123` 管理员、`user` / `user123` 普通用户，
  ...
  **前端裁剪界面从来不是安全边界**。演示口径下这套鉴权是够用的，
  ...
  没有 HTTPS、没有刷新令牌、没有锁定策略（库里的口令是 `scrypt` + 每用户随机 salt
  哈希，不是明文；随仓库分发的 `mine.seed.db` 里只有登录页上那两个演示账号）。
